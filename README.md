...
**Windows 本地桌面 Agent：窗口观察、上下文检索、记忆、上下文预算与流式回答**

一个运行在本地 Windows 桌面上的 AI 伙伴。后台把目标窗口截图转成结构化观察，前台由 `ChatAgent` 管理会话、profile、上下文预算与证据检索，通过 `memory.search(query)` 调取本地上下文，并用可追溯证据完成回答。
...
Local Window Copilot 把桌面 Agent 所需的几件事作为主线来实现：窗口观察、上下文编排、记忆检索、流式工具调用、上下文 compact 和调试可观测性。

| 模块 | 当前实现 | 面试时可讲的工程点 |
|---|---|---|
| 桌面 Agent | Windows 透明置顶悬浮窗 + 独立对话窗 | 低打扰交互、状态机、按需重绘、截图排除、前后台协同 |
| 观察线 | 截图 -> MiniCPM-V 结构化观察 -> SQLite | VLM prompt 契约、截图 metadata、截图轮转、可审计视觉证据 |
| 对话线 | `ChatAgent` + `memory.search(query)` | 工具调用、上下文分层、预算拦截、probe->stream 职责分离 |
| 记忆模块 | RuntimeStore + `memory:items` + `chat_history_fts` + rolling summary | 短期记忆、跨会话检索、滚动 compact、SQLite FTS5/BM25 排名 |
...
上下文管理线
  messages -> rough token estimate -> segment report
  -> request guard before model call
  -> rolling summary for covered history
  -> compact status / manual compact API
...
上下文装配前会生成可审计的预算报告，所有请求在发给模型前先经过预算检查。估算器只依赖标准库，按 CJK、ASCII、JSON 符号、图片 part 和 tool calls 分桶统计，适合本地部署场景下的稳定预检。

compact 采用“完整存储、装配裁剪”的原则：原始对话历史和 FTS5 索引保留，模型输入侧保留最近 tail 原文，把更早且已覆盖的 session 合并进 rolling summary。自动 compact 在每轮对话结束后检查触发条件，手动 compact 可通过 API 或调试界面调用。

| 能力 | 当前状态 |
|---|---|
| rough token estimator | 已接入 context preview 与请求前拦截 |
| ContextAssembler report | 输出 segment、totals、actions 与 over_limit 状态 |
| tool result budget | 对工具结果做分项预算与裁剪 |
| rolling summary state | RuntimeStore 持久化 summary、lock 与 metrics |
| compact planner/executor | 支持自动检查与手动执行 |
...
│  Idle / Observing / Analyzing / Privacy / Error / Chat        │
│  dirty render / async callback queue / screenshot exclusion   │
└───────────────────────────┬──────────────────────────────────┘
                            │ FastAPI
                            ▼
┌──────────────────────────────────────────────────────────────┐
...
│    ├─ probe -> stream(+tools)                                │
│    ├─ memory.search(query)                                   │
│    ├─ rolling compact summary                                │
│    └─ runtime trace                                          │
│                                                              │
│  Observation pipeline                                         │
│    ├─ window capture + screenshot rotation                    │
│    ├─ MiniCPM-V structured observation                        │
...
| 桌面 UI | Python + Win32 / 透明置顶悬浮窗 / 独立聊天窗 / PNG mascot states |
...
| `assistant:chat:rolling_summary:v1` | 已 compact 历史摘要 |
...
- [x] 上下文预算预检、工具结果预算与 rolling compact
- [x] 桌面浮窗按需重绘与截图轮转
- [ ] 演示 GIF / 视频录制
- [ ] 多显示器与窗口选择体验增强
- [ ] Rive 动画迁移
